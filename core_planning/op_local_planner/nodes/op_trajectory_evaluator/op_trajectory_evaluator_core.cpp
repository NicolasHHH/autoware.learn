...
void TrajectoryEval::callbackGetPredictedObjects(const autoware_msgs::DetectedObjectArrayConstPtr& msg)
{
  ...
  for(unsigned int i = 0 ; i <msg->objects.size(); i++)
  {
    ...
    autoware_msgs::DetectedObject det_obj = msg->objects.at(i);

    tf::StampedTransform tf_listen_pose_;
		PlannerHNS::ROSHelpers::GetTransformFromTF("map", "velodyne", tf_listen_pose_);

    tf::Pose tf_pose;
    tf::poseMsgToTF(msg->objects.at(i).pose,tf_pose);

    tf_pose = tf_listen_pose_*tf_pose;

    geometry_msgs::Pose ros_pose;
      tf::poseTFToMsg(tf_pose, ros_pose);

    det_obj.pose = ros_pose;
    ...
    for(unsigned int j=0; j < det_obj.convex_hull.polygon.points.size(); j++)
    {
      ...
      geometry_msgs::Pose convex_hull_pose;
      tf::Pose tf_convex_hull_pose;
      convex_hull_pose.position.x = det_obj.convex_hull.polygon.points.at(j).x;
      convex_hull_pose.position.y = det_obj.convex_hull.polygon.points.at(j).y;
      convex_hull_pose.position.z = det_obj.convex_hull.polygon.points.at(j).z;
      convex_hull_pose.orientation.x = 0;
      convex_hull_pose.orientation.y = 0;
      convex_hull_pose.orientation.z = 0;
      convex_hull_pose.orientation.w = 1;
      tf::poseMsgToTF(convex_hull_pose,tf_convex_hull_pose);
      tf_convex_hull_pose = tf_listen_pose_*tf_convex_hull_pose;

      det_obj.convex_hull.polygon.points.at(j).x = tf_convex_hull_pose.getOrigin().x();
      det_obj.convex_hull.polygon.points.at(j).y = tf_convex_hull_pose.getOrigin().y();
      det_obj.convex_hull.polygon.points.at(j).z = tf_convex_hull_pose.getOrigin().z();

      point.x = det_obj.convex_hull.polygon.points.at(j).x;
          point.y = det_obj.convex_hull.polygon.points.at(j).y;
          point.z = det_obj.convex_hull.polygon.points.at(j).z;
          myPolygon.polygon.points.push_back(point);

      //std::cout<<"obj.center.pos.x: "<<det_obj.convex_hull.polygon.points.at(j).x<<std::endl;
      //std::cout<<"obj.center.pos.y: "<<det_obj.convex_hull.polygon.points.at(j).y<<std::endl;
      //std::cout<<"obj.center.pos.z: "<<det_obj.convex_hull.polygon.points.at(j).z<<std::endl;
    }
    
    if(1)
    {
      PlannerHNS::ROSHelpers::ConvertFromAutowareDetectedObjectToOpenPlannerDetectedObject(det_obj, obj);
      m_PredictedObjects.push_back(obj);
    }
    
    /*
    if(msg->objects.at(i).id > 0) //note-tianyu 有时候可能要把它注释掉，否则不满足判断条件
    {
  ...
  while (ros::ok())
  {
    ...
    if(bNewCurrentPos && m_GlobalPaths.size()>0)
    {
      ...
      for(unsigned int i = 0; i < m_GlobalPathsToUse.size(); i++)
      {
        ...
        //note-tianyu 这里类似于velocity_set的做法，将m_GlobalPathsToUse根据cur_pose的closest index来截取其中一部分，即m_GlobalPathSections
      ...
      if(m_GlobalPathSections.size()>0)
      {
        if(m_bUseMoveingObjectsPrediction)
          tc = m_TrajectoryCostsCalculator.DoOneStepDynamic(m_GeneratedRollOuts, m_GlobalPathSections.at(0), m_CurrentPos,m_PlanningParams,  m_CarInfo,m_VehicleStatus, m_PredictedObjects, m_CurrentBehavior.iTrajectory);
        	...
        	//note-tianyu 基于动态预测或者静态障碍的轨迹权重计算, 返回的tc代表当前最优轨迹
